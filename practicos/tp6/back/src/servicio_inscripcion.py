...
class ServicioInscripcion:
    ...
    def __init__(
        self,
        setup_actividades: Dict,
        turnos_disponibles: List[Turno],
    ...
    def _persistir_inscripcion(self, inscripcion: Inscripcion):
        ...
        if hasattr(self.repo, "inscripciones") and isinstance(
            self.repo.inscripciones, list
        ):
    ...
    def inscribir(
        ...
        if (
            ...
            raise ErrorParqueCerrado(
                "El parque está cerrado los días lunes, 1 de enero y 31 de diciembre."
            )
        ...
        if dias_anticipacion >= 3:
            raise ErrorAnticipacion(
                "La inscripción no puede realizarse con 3 o más días de anticipación."
            )

        # 5) Horario de apertura/cierre
        if turno.hora < self.horario_apertura or turno.hora > self.horario_cierre:
            raise ErrorHorarioInvalido(
                "Horario fuera del rango de apertura del parque."
            )
        ...
        for p in participantes:
            if getattr(p, "edad", None) is None or p.edad < 0 or p.edad > 150:
                raise ErrorRestriccionEdad(
                    "Edad inválida: debe ser mayor a 0 y no superar 150."
                )
        ...
        if reglas and reglas.get("edad_minima") is not None:
            ...
            for p in participantes:
                if getattr(p, "edad", None) is None or p.edad < edad_min:
                    raise ErrorRestriccionEdad(
                        "Al menos un participante no cumple la edad mínima requerida."
                    )
        ...
        for ins in inscripciones_existentes:
            ...
            if turno_exist.fecha == turno.fecha and turno_exist.hora == turno.hora:
                ...
                dnis_existentes = {
                    getattr(v, "dni", None) for v in visitantes_existentes
                }
                for p in participantes:
                    if getattr(p, "dni", None) in dnis_existentes:
                        raise ErrorChoqueHorario(
                            "El visitante ya tiene otra actividad en el mismo horario."
                        )
        ...
        if turno.id in self.turnos_disponibles:
            self.turnos_disponibles[turno.id].cupo_ocupado = getattr(
                turno, "cupo_ocupado", cupo_ocupado_actual + num_nuevos
            )
