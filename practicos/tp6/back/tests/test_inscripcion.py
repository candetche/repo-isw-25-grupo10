...
def setup_inscripcion(setup_actividades):
    ...
    FECHA_LUNES_CERRADO = FECHA_HOY + timedelta(
        days=dias_hasta_lunes if dias_hasta_lunes != 0 else 7
    )  # Evita el lunes de hoy si es hoy
    ...
    turno_jardineria_poco_cupo = Turno(
        id=301,
        actividad_nombre="Jardinería",
        fecha=FECHA_HOY,
        hora=time(10, 0),
        cupo_ocupado=10,
    )
    turno_invalido_cerrado = Turno(
        id=999,
        actividad_nombre="Tirolesa",
        fecha=FECHA_HOY,
        hora=time(18, 30),
    ...
    turno_lunes_cerrado = Turno(
        id=110,
        actividad_nombre="Tirolesa",
        fecha=FECHA_LUNES_CERRADO,
        hora=time(14, 0),
        cupo_ocupado=0,
    )
    turno_25_dic = Turno(
        id=120,
        actividad_nombre="Tirolesa",
        fecha=date(FECHA_HOY.year, 12, 25),
        hora=time(14, 0),
        cupo_ocupado=0,
    )
    turno_1_ene = Turno(
        id=130,
        actividad_nombre="Tirolesa",
        fecha=date(FECHA_HOY.year + 1, 1, 1),
        hora=time(14, 0),
        cupo_ocupado=0,
    )
    turno_anticipacion_excesiva = Turno(
        id=140,
        actividad_nombre="Tirolesa",
        fecha=FECHA_3_DIAS_ANTICIPACION,
        hora=time(14, 0),
        cupo_ocupado=0,
    )
    turno_fecha_pasada = Turno(
        id=160,
        actividad_nombre="Tirolesa",
        fecha=FECHA_AYER,
        hora=time(10, 0),
    ...
    turnos_disponibles = [
        turno_tirolesa_con_cupo,
        turno_palestra_sin_cupo,
        turno_jardineria_poco_cupo,
        turno_fecha_pasada,
    ]

    # INSCRIPCIONES EXISTENTES (Para Test 8)
    turno_ana_ya_existente = Turno(
        id=500,
        actividad_nombre="Jardinería",
        fecha=FECHA_HOY,
        hora=time(14, 0),
    ...
    inscripcion_ana = Inscripcion(
        turno=turno_ana_ya_existente,
        visitantes=[visitante_ana_reserva],
        total_personas=1,
        acepta_terminos=True,
        email_contacto=EMAIL_VALIDO,
    )
    ...
    servicio = ServicioInscripcion(
        setup_actividades, turnos_disponibles, repo, fecha_actual=FECHA_HOY
    )
...
def test_inscripcion_sin_talle_no_requerido_pasa(setup_inscripcion):
    ...
    inscripcion = s["servicio"].inscribir(
        ...
        participantes=[
            s["visitante_ema_sin_talle"]
        ],  # Talle es None, pero la regla dice NO se requiere
        acepta_terminos=True,
        email_contacto=s["EMAIL_VALIDO"],
    )
...
def test_inscripcion_sin_talle_requerido_falla(setup_inscripcion):
    ...
    with pytest.raises(ErrorFaltaTalle):
        s["servicio"].inscribir(
            ...
            participantes=[
                s["visitante_ema_sin_talle"]
            ],  # Talle es None, pero la actividad lo requiere
            acepta_terminos=True,
            email_contacto=s["EMAIL_VALIDO"],
        )
...
def test_inscripcion_concurrencia_falla(setup_inscripcion):
    ...
    with pytest.raises(ErrorChoqueHorario):
        s["servicio"].inscribir(
            turno=s[
                "turno_tirolesa_con_cupo"
            ],  # Mismo horario que la reserva existente de Ana (14:00)
...
def test_inscripcion_multiple_sin_cupo_falla(setup_inscripcion):
    ...
    participantes_insuficientes = [
        s["visitante_beto_valido"],
        s["visitante_ana_reserva"],
        s["visitante_ceci_edad_8"],
    ]  # Total: 3 personas
